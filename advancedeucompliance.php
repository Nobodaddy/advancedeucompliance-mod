 ...
 *
 * @author     PrestaShop SA <contact@prestashop.com> / Rainer Schmidt <translations@cenith.net>
 * @copyright  2007-2018 PrestaShop SA & eleazar
...
class Advancedeucompliance extends Module
{
    ...
    public function createConfig()
    {
        ...
        foreach ($langs as $lang) {
			$delivery_time_available_values[(int)$lang->id] = '';
			$delivery_time_oos_values[(int)$lang->id] = '';
    ...
    */
    ...
    public function hookOverrideTOSDisplay($param)
    {
        ...
        Media::addJsDef(array('aeuc_has_virtual_products' => (bool)$has_virtual_product, 'aeuc_virt_prod_err_str' => Tools::htmlentitiesUTF8($this->l('Please check "Revocation of virtual products" box first !', 'advancedeucompliance'))));
    ...
    public function hookDisplayProductPriceBlock($param)
    {
        ...
        $hookType = $param['type'];
		$cacheId = sha1($hookType.$idShop.$idLang);
        ...
        if ($param['type'] == 'before_price' && (bool)Configuration::get('AEUC_LABEL_COMBINATION_FROM') === true) {
            ...
            return $this->dumpHookDisplayProductPriceBlock($smartyVars, sha1($hookType.$idShop.$idLang));
                    }
                }
    ...
    private function dumpHookDisplayProductPriceBlock(array $smartyVars, $cacheId)
    {
        $this->context->smarty->assign(array('smartyVars' => $smartyVars));
		$this->context->controller->addJS($this->_path . 'views/js/fo_aeuc_tnc.js', true);
        $idShop = str_pad(Context::getContext()->shop->id, 3);
		...
		return $this->display(__FILE__, 'hookDisplayProductPriceBlock.tpl', sha1($cacheId.$idShop.$idLang));
    }
    ...
    protected function _postProcess()
    {
       ...
       <!-- adapted to PHP 7.2 by eleazar -->
            return (count($this->_errors) ? $this->displayError($this->_errors) : '') .
                  (is_array($this->_warnings) && count($this->_warnings) ? $this->displayWarning($this->_warnings) : '') .
